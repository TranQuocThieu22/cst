...
import { ReportLunchPayment, Member } from './tien-an-trua-DT'
@Component({
...
export class TienAnTruaTheoThangComponent implements OnInit {
  ...
  calculateTotal(type: string) {
    let total = 0;
    switch (type) {
      case 'total_IndividualDayOff':
        total = this.AQLunchPaymentReport.reduce((acc, report) => acc + report.total_IndividualDayOff, 0);
        break;
      case 'total_WorkingOnline':
        total = this.AQLunchPaymentReport.reduce((acc, report) => acc + report.total_WorkingOnline, 0);
        break;
      case 'total_CommissionDay':
        total = this.AQLunchPaymentReport.reduce((acc, report) => acc + report.total_CommissionDay, 0);
        break;
      case 'total_AQDayOff':
        total = this.AQLunchPaymentReport.reduce((acc, report) => acc + report.total_AQDayOff, 0);
        break;
      case 'actual_workingDay':
        total = this.AQLunchPaymentReport.reduce((acc, report) => acc + report.actual_workingDay, 0);
        break;
      default:
        break;
    }
